...
- ✅ **玻璃态卡片**：内容卡片使用毛玻璃效果（backdrop-filter），背景图片可透过
- ✅ **深色模式适配**：深色模式下自动调整叠加层和卡片颜色
- ✅ **响应式**：自动适配不同屏幕尺寸

### 叠加透明度：
- **所有部分统一使用**: **72% 不透明度** (0.72)
- 与英雄区域相同的视觉效果
- 背景图片清晰可见，文字依然清晰可读
- 深色模式自动适配相同透明度

### 卡片透明度：
- **服务卡片 (Services)**: 85% 不透明度，悬停时 95%
- **流程步骤 (Workflow)**: 85% 不透明度，悬停时 95%
- **信任卡片 (Trust)**: 85% 不透明度，悬停时 95%
- **联系卡片 (Contact)**: 85% 不透明度（卡片），90% 不透明度（表单）
- **毛玻璃效果**: `backdrop-filter: blur(10px)` 让背景图片透过卡片
